...
import { isSupportedFunction } from "./common"
...
export class TypeScriptExpressionTransformer {
  ...
  private invocation(expr: InvocationExpr, normalizeUndefined: boolean) {
    ...
    const isStdFunc =
      isFromStdlib(expr.function.ref) || isSupportedFunction(expr.function.ref)
  ...
  @func("jsonContains")
  private _jsonContains(args: Expression[], normalizeUndefined: boolean) {
    const field = this.transform(args[0], false)
